...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // 2. Generate and set CSRF token for all routes
    ...
    // 4. Auth route handling
    if (isAuthRoute(pathname)) {
      if (session) {
        const role = session.user?.user_metadata?.role as UserRole | undefined;
        return NextResponse.redirect(getRoleBasedRedirect(role) || '/dashboard');
      }
      return response;
    }
    ...
    // 6. Role validation
    const role = session.user?.user_metadata?.role as UserRole | undefined;
    if (!role || !isRoleProtected(pathname, role)) {
...
// // This middleware handles authentication, authorization, and security headers for a Next.js application
// // It uses Supabase for session management and applies security headers to all responses
// import { NextResponse } from "next/server";
// import type { NextRequest } from "next/server";
// import { createServerSupabaseClient } from "@/lib/supabase/server";
